...
if (-not (Get-Command node -ErrorAction SilentlyContinue)) {
  ...
  Write-Host "Using preinstalled Node $(node --version)"
  # pull the package into the cache so Save-ChocoPackage still works
  choco download "$nodejs" --version="20.5.1" --require-checksums `
                --output-directory "${PSScriptRoot}\..\tmp\chocolatey\$nodejs" `
                --no-progress
}
